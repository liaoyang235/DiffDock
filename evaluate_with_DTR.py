...
if __name__ == '__main__':
    ...
    parser.add_argument('--specific_model', type=str, default=None, help='Specific model to use for inference')

    parser.add_argument('--config', type=FileType(mode='r'), default=None)
    parser.add_argument('--model_dir', type=str, default='workdir/test_score/DTR_0.6_wo_alpha_4.0', help='Path to folder with trained score model and hyperparameters')
    ...
    if not args.no_model:
        ...
        if args.specific_model is not None:
            eval_models.append(args.specific_model)
        else:
            for _ in os.listdir(args.model_dir):
                if _.endswith('_model.pt') and _ != 'last_model.pt':
                    eval_models.append(_)
        arranged_models = sorted(eval_models, key=lambda x: int(x.split('_')[1]))

        for _ in arranged_models:
            state_dict = torch.load(f'{args.model_dir}/{_}', map_location=torch.device('cpu'))
